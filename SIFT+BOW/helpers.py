...
class Bag_of_words_helpers:
	...
	def developVocabulary(self,n_label,n_names,n_images, descriptor_list, kmeans_ret = None):
	...
	def getFiles(self, path):
		...
		for each in glob(path + "*"):
			...
			for imagefile in glob(path+word+"/*"):
				...
				#adding name of images
				names[count] = imagefile
				im = cv2.imread(imagefile, 0)
				imlist[word].append(im)
				count +=1 

		return [imlist, count, names]
